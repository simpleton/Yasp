...
public class MMapTable extends Table {
  ...
  public MMapTable(
    ...
    checkArgument(
        fileChannel.size() <= Integer.MAX_VALUE,
        "File must be smaller than %s bytes",
        Integer.MAX_VALUE
    );
  }
  ...
  public static ByteBuffer read(MappedByteBuffer data, int offset, int length) throws IOException {
    ...
    ByteBuffer block = (ByteBuffer) data.duplicate()
  ...
  protected Footer init() throws IOException {
    ...
    Slice footerSlice =
        Slices.copiedBuffer(data, (int) size - Footer.ENCODED_LENGTH, Footer.ENCODED_LENGTH);
  ...
  @SuppressWarnings({
      "NonPrivateFieldAccessedInSynchronizedContext", "AssignmentToStaticFieldFromInstanceMethod"
  })
  @Override
