...
public class DbImplTest {
  ...
  static final String DOES_NOT_EXIST_FILENAME = "/foo/bar/doowop/idontexist";
  static final String DOES_NOT_EXIST_FILENAME_PATTERN = ".foo.bar.doowop.idontexist";
  final ArrayList<DbStringWrapper> opened = new ArrayList<>();
  ...
  public void testBackgroundCompaction() throws Exception {
    ...
    for (int i = 0; i < 200000 * STRESS_FACTOR; i++) {
      db.put(
          randomString(random, 64).getBytes(),
          new byte[] { 0x01 },
  ...
  public void testIteratorSingle() throws Exception {
    ...
    assertSequence(db.iterator(), new AbstractMap.SimpleImmutableEntry<>("a", "va"));
  }
  ...
  public void testIteratorMultiple() throws Exception {
    ...
    assertSequence(iterator,
        new AbstractMap.SimpleImmutableEntry<>("a", "va"),
        new AbstractMap.SimpleImmutableEntry<>("b", "vb"),
        new AbstractMap.SimpleImmutableEntry<>("c", "vc")
    );
  ...
  public void testApproximateSizes() throws Exception {
    DbStringWrapper db = new DbStringWrapper(
        new Options().writeBufferSize(100000000).compressionType(NONE),
        databaseDir
    );
  ...
  public void testIteratorPinsRef() throws Exception {
    ...
    assertSequence(iterator, new AbstractMap.SimpleImmutableEntry<>("foo", "hello"));
  }
  ...
  public void testSingleEntrySingle() throws Exception {
    DbStringWrapper db = new DbStringWrapper(new Options(), databaseDir);
    testDb(db, new AbstractMap.SimpleEntry<>("name", "dain sundstrom"));
  }
  ...
  public void testMultipleEntries() throws Exception {
    ...
    List<Entry<String, String>> entries = asList(
        (Entry<String, String>) new AbstractMap.SimpleImmutableEntry<>("beer/ale", "Lagunitas  Little Sumpin’ Sumpin’"),
        new AbstractMap.SimpleImmutableEntry<>("beer/ipa", "Lagunitas IPA"),
        new AbstractMap.SimpleImmutableEntry<>("beer/stout", "Lagunitas Imperial Stout"),
        new AbstractMap.SimpleImmutableEntry<>("scotch/light", "Oban 14"),
        new AbstractMap.SimpleImmutableEntry<>("scotch/medium", "Highland Park"),
        new AbstractMap.SimpleImmutableEntry<>("scotch/strong", "Lagavulin")
    );
  ...
  @Test(expectedExceptions = IllegalArgumentException.class, expectedExceptionsMessageRegExp =
      "Database directory '"
      + DOES_NOT_EXIST_FILENAME_PATTERN
      + "'.*")
  ...
  @Test(
      ...
      expectedExceptionsMessageRegExp = "Database directory.*is not a directory"
  )
  ...
  public void testCustomComparator() throws Exception {
    DbStringWrapper db =
        new DbStringWrapper(new Options().comparator(new ReverseDBComparator()), databaseDir);

    List<AbstractMap.SimpleImmutableEntry<String, String>> entries = asList(
        new AbstractMap.SimpleImmutableEntry<>("scotch/strong", "Lagavulin"),
        new AbstractMap.SimpleImmutableEntry<>("scotch/medium", "Highland Park"),
        new AbstractMap.SimpleImmutableEntry<>("scotch/light", "Oban 14"),
        new AbstractMap.SimpleImmutableEntry<>("beer/stout", "Lagunitas Imperial Stout"),
        new AbstractMap.SimpleImmutableEntry<>("beer/ipa", "Lagunitas IPA"),
        new AbstractMap.SimpleImmutableEntry<>("beer/ale", "Lagunitas  Little Sumpin’ Sumpin’")
    );
  ...
  private void testDb(DbStringWrapper db, List<Entry<String, String>> entries) throws IOException {
    ...
    for (Entry<String, String> entry : entries) {
      List<Entry<String, String>> nextEntries =
  ...
  private static class ReverseDBComparator implements DBComparator {
    ...
    public int compare(byte[] sliceA, byte[] sliceB) {
      // reverse order
      Arrays.equals(sliceA, sliceB);
      return -(Utils.compare(sliceA, sliceB));
    }
  ...
  private static class StringDbIterator implements SeekingIterator<String, String> {
    ...
    private Entry<String, String> adapt(Entry<byte[], byte[]> next) {
      return new AbstractMap.SimpleImmutableEntry<>(
          new String(next.getKey(), UTF_8),
          new String(next.getValue(), UTF_8)
      );
    }
  }
  ...
  private class DbStringWrapper {
    ...
    List<String> allEntriesFor(String userKey) {
      ...
      for (Entry<InternalKey, Slice> entry : db.internalIterable()) {
        ...
        if (entryKey.equals(userKey)) {
          if (entry.getKey().getValueType() == ValueType.VALUE) {
            list.add(entry.getValue().toString(UTF_8));
          } else {
            list.add("DEL");
          }
        }
